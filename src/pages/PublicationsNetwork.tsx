...
export default function PublicationsNetwork() {
  ...
  const heroSubtitle = String(publicationsNetworkHero?.subtitle ?? t('publicationsNetwork.subtitle'))
  const heroTitle = String(publicationsNetworkHero?.title ?? t('publicationsNetwork.title'))
  const heroDescription = String(publicationsNetworkHero?.description ?? t('publicationsNetwork.description'))
    ...
    <section className="relative min-h-[90vh] overflow-hidden pb-[env(safe-area-inset-bottom)]">
      ...
      <div className="absolute bottom-0 left-0 right-0 z-10 flex justify-center px-4 pb-6 md:pb-8 pointer-events-none">
        <motion.div
          ...
          className="flex flex-wrap items-center justify-center gap-x-6 gap-y-3 py-3 px-5 md:px-6 rounded-2xl bg-gradient-to-br from-white/95 via-white/90 to-[#f4f4f7] shadow-xl shadow-slate-300/30 border border-white/60 backdrop-blur-xl"
        >
